...
class ExperienceBuffer:
    ...
    def __init__(self, env_info, algo_info, device, aux_tensor_dict=None):
        ...
        self.num_agents = env_info.get('agents', 2)
    ...
    def _init_from_env_info(self, env_info):
        ...
        print("self.obs_base_shape",self.obs_base_shape)
        print("action shape in exp:", self.tensor_dict['actions'].size())
        print("action box: ",gym.spaces.Box(low=0, high=1,shape=self.actions_shape, dtype=np.float32))
    ...
    def update_data(self, name, index, val):
        print("val shape in experince",val.size())
        if type(val) is dict:
