...
for i in range(num_envs):
    ...
    arrs= map.get_lines_positions(i % 5)
    ...
    for pos,length,p1,p2 in zip(poss,lenths,arrs[0],arrs[1]):
        ...
        box_pose.p = gymapi.Vec3(pos[0], pos[1], 0.0)

        theta = math.atan((p2[1] - p1[1])/(p2[0] - p1[0]))  # math.pi/2.0
        box_pose.r = gymapi.Quat(0*math.sin(theta / 2), 0*math.sin(theta / 2),  math.sin(theta / 2), 1*math.cos(theta / 2))
        ...
        #p0_handle = gym.create_actor(env, capsule_asset, pose, "MyActor", i, 1)
